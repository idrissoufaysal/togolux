...
export function AdminDashboardShell({
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      if (res.success) {
        // Force refresh the router or hard redirect to login to clean server session context
        ...
        <div className="p-4 border-t border-white/10 space-y-3 bg-black/15">
          <div className="flex items-center gap-3 px-2">
            ...
            <div className="min-w-0">
              ...
              <span className="text-[10px] text-white/50 block truncate">{userEmail}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-3 py-2 bg-white/5 hover:bg-red-950/20 text-white/80 hover:text-red-400 border border-white/10 hover:border-red-950/30 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              <span>{isLoggingOut ? '...' : 'Quitter'}</span>
            </button>
          </div>
        </div>
      </aside>
        ...
        <header className="hidden md:flex h-20 items-center justify-between px-8 bg-white/80 backdrop-blur-md border-b border-[#dec1ac]/60 shrink-0 sticky top-0 z-40">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              <button className="text-[#322214] hover:text-[#c5a373] transition-colors relative" aria-label="Notifications">
